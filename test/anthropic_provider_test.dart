...
import 'package:teacher/features/ai_teacher/data/anthropic_api_client.dart';
import 'package:teacher/features/ai_teacher/data/anthropic_provider.dart';
...
/// Builds the Anthropic Messages envelope: the actual structured payload
/// is a JSON-encoded string nested inside `content[0].text`.
String _envelope(String innerText, {String stopReason = 'end_turn'}) =>
    jsonEncode({
      'content': [
...
AnthropicProvider _providerWith(_FakeAdapter adapter) {
  final dio = Dio();
...
void main() {
  ...
  group('AnthropicProvider.generateModuleTest', () {
  ...
  group('error mapping', () {
    ...
    test(
      'a stop_reason of refusal with empty content throws AIUnavailableException without crashing',
      () async {
        final adapter = _FakeAdapter([
          (_) => _jsonResponse(
