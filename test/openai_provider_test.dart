...
import 'package:teacher/features/ai_teacher/data/openai_compatible_api_client.dart';
import 'package:teacher/features/ai_teacher/data/openai_compatible_provider.dart';
...
/// Builds the OpenAI-compatible Chat Completions envelope: the actual
/// structured payload is a JSON-encoded string nested inside
...
OpenAiCompatibleProvider _providerWith(
  ...
  final client = OpenAiCompatibleApiClient(
    apiKey: 'test-api-key',
    baseUrl: baseUrl,
...
void main() {
  group('OpenAiCompatibleProvider.teach', () {
    ...
    test(
      'the same success path works identically for a DeepSeek-labeled provider',
      () async {
        final adapter = _FakeAdapter([
          (_) => _jsonResponse(
            _envelope(
              jsonEncode({'explanation': 'A closure captures variables by reference.'}),
            ),
          ),
        ]);
  ...
  group('OpenAiCompatibleProvider.generateModuleTest', () {
